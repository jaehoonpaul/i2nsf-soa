...
def create_security_resource_group_capability(request):
    return render(request, 'security/capability_modal.html', {"modal_title": _("Capability 직접입력")})
    ...
    if request.is_ajax() and request.method == 'POST':
        ...
        try:
            if security_info.get("capability_xml"):
                ...
                (out, err) = insert_result.communicate()
                logger.info("\n\n\ncall result: {}\n\n\n".format(out))
            else:
            ...
            insert_result = {"success": {"message": _(u"보안서비스로 등록되었습니다."), "title": _("성공")}}
        ...
        return render(request, 'security/info.html', {"title": _(u"등록")})
...
def modify_security_resource_group(request, security_id):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'security/info.html', {"title": _("수정"), "security_id": security_id})
...
def get_security_resource_list(request):
    ...
    for security_resource in security_resource_list:
        ...
        if security_resource.get("capability_xml"):
            file_path = security_resource.get("capability_xml")
            if path.isfile(file_path):
                with open(file_path, 'r') as f:
                    security_resource["xml_contents"] = f.read()
...
def delete_security_resource(request, security_id):
    ...
    if request.method == 'POST':
        ...
        db_port = request.session.get("domain_db_port")
        conn = SOAControlDomainDBConnector(db_nm=db_nm, db_user=db_user, db_pass=db_pass, db_ip=db_ip, db_port=db_port)
        delete_security_resource = conn.delete_security_resource_group({"domain_name": domain_name, "security_id": security_id})
        result = {"success": {"message": _("보안서비스를 삭제했습니다."), "title": _("성공")}}
        return JsonResponse(result)
    ...
    if request.is_ajax() and request.method == 'POST':
        ...
        try:
            ...
            if len(select_security_resource) > 0:
                ...
                raise Exception(security_id + _("은 존재하지 않습니다."))
        except Exception as e:
        ...
        return render(request, 'security/info.html', {"title": _("조회"), "security_id": security_id})
                        ...
                        if len(used_security_group_id_list) > 0:
                            ...
                            try:
                                ...
                                if len(select_security_resource) > 0:
                                    ...
                                    raise Exception(used_security_group_id + _(u"은 존재하지 않습니다."))
                            except Exception as e:
                                result = {"error": {"message": str(e), "title": "error"}}
                    else:
                        result = {"error": {"message": _("해당 서버에는 보안정보가 없습니다."), "title": "error"}}
                else:
                    result = {"error": {"message": _("해당 서비스에는 보안정보가 없습니다."), "title": "error"}}
            else:
                result = {"error": {"message": _("해당 서버가 없습니다."), "title": "error"}}
...
def send_capability_xml(request):
    try:
        if request.POST.get("capability_xml"):
            file_path = path.join(STATIC_DIR, "media")
            file_path = path.join(file_path, request.POST.get("capability_xml").replace("/files/download/", ""))

            scr_api = SecurityControllerRestAPI()
            insert_result = scr_api.insert_nsf(file_path)
            (out, err) = insert_result.communicate()
        insert_result = {"success": {"message": _(u"성공적으로 전송하였습니다."), "title": _(u"전송")}}
    except Exception as e:
        insert_result = {"error": {"message": str(e), "title": "error"}}
    return JsonResponse(insert_result)
