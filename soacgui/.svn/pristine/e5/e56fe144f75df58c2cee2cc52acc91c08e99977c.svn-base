    ...
    if request.method == "GET":
        ...
        if result.get("success"):
            ...
            if rule.get("start_time") and rule.get("end_time"):
                json_param["ietf-i2nsf-cfi-policy:policy"]["rule"][0]["event"] = {
                    "time-information": {
...
def create_endpoint(request):
    ...
    if request.is_ajax() and request.method == "POST":
        ...
        return render(request, 'service/intent/endpoint/create_modal.html', {"modal_title": _(u"Endpoint 생성")})
...
def update_endpoint(request, endpoint_key):
    ...
    if request.method == "GET":
        ...
        policy = metadata["ietf-i2nsf-cfi-policy:policy"]
        if endpoint.get("endpoint_type") == "url":
            endpoint["content"] = policy["threat-prevention"]["payload-content"][0]["content"][0]
        else:
            range_ip_address = policy["endpoint-group"][endpoint["endpoint_type"] + "-group"][0]["range-ip-address"][0]
            endpoint["start_ip_address"] = range_ip_address.get("start-ip-address")
            endpoint["end_ip_address"] = range_ip_address.get("end-ip-address")

        return render(request, 'service/intent/endpoint/create_modal.html', {"modal_title": _(u"Endpoint 수정"), "endpoint": endpoint})
    elif request.is_ajax() and request.method == "POST":
        conn = SOAControlDBConnector.getInstance()
        params = (
            request.POST.get("name"),
